        ...
        <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div class="dashboard-card mb-6">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div>
                        <h1 class="text-2xl font-bold text-white mb-2">Editar Pedido #{{ $pedido->id }}</h1>
                        <p class="text-zinc-300">Modifica los datos del pedido</p>
                    </div>
                ...
                <form action="{{ route('pedido-mesas.update', $pedido) }}" method="POST"
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div class="dashboard-card">
                        ...
                        <div class="space-y-4">
                            <div>
                                <label class="block text-zinc-300 font-medium mb-2">Mesa *</label>
                                <select name="mesa_id" required class="w-full bg-zinc-800 border border-zinc-700 rounded-lg px-3 py-2 text-white focus:ring-2 focus:ring-blue-500">
                                    <option value="">Seleccionar mesa</option>
                                    @foreach($mesas as $mesa)
                                        <option value="{{ $mesa->id }}" {{ $pedido->mesa_id == $mesa->id ? 'selected' : '' }}>{{ $mesa->nombre }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div>
                ...
                <div class="flex flex-col sm:flex-row justify-end gap-4 mt-6">
                    <a href="{{ route('pedido-mesas.index') }}" class="bg-zinc-700 hover:bg-zinc-600 text-white py-3 px-6 rounded-lg transition-colors text-center">
                        Cancelar
                    </a>
    ...
    window.pedidoForm = function(items) {
        return {
            ...
            handleProductsSelected(event) {
                console.log('Productos recibidos en edit:', event.detail); // Para debug
                this.items = [...event.detail];
                console.log('Items actualizados:', this.items); // Para debug
            },
