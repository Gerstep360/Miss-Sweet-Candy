{{-- filepath: c:\Users\German\Documents\Proyectos\PHP\Cafeteria\cafeteria\resources\views\cajero\pedido_mesas\index.blade.php --}}
<x-layouts.app :title="__('Pedidos en Mesa')">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div class="dashboard-card mb-8">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div>
                        <h1 class="text-2xl font-bold text-white mb-2">Pedidos en Mesa</h1>
                        <p class="text-zinc-300">Administra los pedidos activos en el sistema</p>
                    </div>
                    @can('crear-ordenes')
                        <a href="{{ route('pedido-mesas.create') }}" class="bg-amber-500 hover:bg-amber-400 text-black font-medium py-2 px-4 rounded-lg transition-colors flex items-center gap-2">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                ...
                <div class="dashboard-card hover:bg-zinc-800/40 transition-colors">
                    <div class="mb-4">
                        <div class="flex items-center gap-3 mb-2">
                            ...
                            <div class="flex-1">
                                <h3 class="text-lg font-semibold text-white">Mesa: {{ $pedido->mesa->nombre ?? '-' }}</h3>
                    ...
                    <div class="flex flex-wrap gap-2">
                        @can('ver-ordenes')
                            <a href="{{ route('pedido-mesas.show', $pedido) }}" 
                        ...
                        @endcan
                        @can('editar-ordenes')
                            <a href="{{ route('pedido-mesas.edit', $pedido) }}" 
                        ...
                        @if($pedido->estado !== 'servido')
                            ...
                            @can('procesar-ordenes')
                                <button type="button"
                                    @click="showModal = true; modalAction = 'procesar'; modalUrl = '{{ route('pedido-mesas.procesar', $pedido) }}'; modalMsg = '¿Enviar este pedido a barra/cocina?';"
                            ...
                            @can('completar-ordenes')
                                <button type="button"
                                    @click="showModal = true; modalAction = 'completar'; modalUrl = '{{ route('pedido-mesas.completar', $pedido) }}'; modalMsg = '¿Marcar este pedido como completado?';"
                        ...
                        @endif
                        @can('anular-ordenes')
                            <form action="{{ route('pedido-mesas.destroy', $pedido) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
                                <button type="button"
                                    @click="showModal = true; modalAction = 'eliminar'; modalUrl = '{{ route('pedido-mesas.destroy', $pedido) }}'; modalMsg = '¿Eliminar este pedido?';"
                ...
                <div class="col-span-full dashboard-card text-center py-12">
                    ...
                    <h3 class="text-white font-medium mb-2">No hay pedidos en mesa</h3>
                    <p class="text-zinc-400 mb-4">Aún no se han registrado pedidos.</p>
                    @can('crear-ordenes')
                    <a href="{{ route('pedido-mesas.create') }}" class="btn-emerald py-2 px-4 rounded-lg transition-colors inline-flex items-center gap-2">
                ...
                <div class="flex justify-end gap-3">
                    ...
                    <form :action="modalUrl" method="POST" class="inline-block">
                        ...
                        <button type="submit"
                            ...
                            :class="modalAction === 'completar' ? 'bg-green-600 hover:bg-green-500' : (modalAction === 'eliminar' ? 'bg-red-600 hover:bg-red-500' : 'bg-purple-600 hover:bg-purple-500')">
                            <span x-text="modalAction === 'completar' ? 'Sí, completar' : (modalAction === 'eliminar' ? 'Sí, eliminar' : 'Sí, procesar')"></span>
                        </button>
                    </form>
                </div>
            </div>
