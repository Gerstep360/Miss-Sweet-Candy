...
<head>
    ...
    @vite(['resources/css/app.css', 'resources/js/app.js', 'resources/css/layouts/app/sidebar.css'])

    {{-- Alpine Components moved to resources/js/alpine-components.js --}}
</head>

<body
    class="min-h-screen bg-zinc-950 text-white antialiased selection:bg-amber-500 selection:text-black relative overflow-x-hidden"
    x-data="mainLayout({{ $unread }})" @resize.window="if(window.innerWidth >= 1024) close()" @keydown.escape.window="close()"
    ...
    <div class="fixed inset-0 z-[-1] pointer-events-none">
        ...
        <div
            class="absolute left-0 right-0 top-0 -z-10 m-auto h-[310px] w-[310px] rounded-full bg-amber-500 opacity-[0.03] blur-[100px]">
        </div>
    </div>
    ...
    <div id="sidebar-overlay" x-cloak
        ...
        :class="mobileOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'" @click="close()">
    </div>
    ...
    <div id="mobile-controls" class="fixed bottom-6 left-6 flex items-center gap-3 lg:hidden z-[2]" x-cloak>
        ...
        <button type="button" @click="toggle()"
            ...
            <svg class="w-7 h-7 text-zinc-300 group-hover:text-white" fill="none" stroke="currentColor"
                viewBox="0 0 24 24">
        ...
        <button id="btn-open-notifs-floating" type="button" @click="$dispatch('open-notifications')"
            ...
            <svg class="w-7 h-7" :class="unread > 0 ? 'text-amber-400' : 'text-zinc-300'" fill="none"
            ...
            {{-- Badge Contador --}}
            <span
                class="absolute -top-1 -right-1 flex h-5 w-5 items-center justify-center rounded-full bg-red-600 border-2 border-zinc-950 text-[10px] font-bold text-white shadow-sm"
                x-show="unread > 0" :class="unread > 0 ? 'animate-pulse' : ''"
