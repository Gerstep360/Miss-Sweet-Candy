...
<div x-data="notificationSystem"
     @open-notifications.window="triggerPanel()"
     x-cloak>
    ...
    <div x-show="openDesktop" 
         x-transition.opacity.duration.300ms
         @click="openDesktop = false"
    ...
    <aside x-show="openDesktop"
           ...
           style="display: none;" 
           @keydown.escape.window="openDesktop = false">
        
        {{-- Header --}}
        <div class="px-5 py-4 border-b border-zinc-800 flex items-center justify-between bg-zinc-900/95 backdrop-blur-sm shrink-0">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-[11px] text-zinc-500" x-text="unreadCount > 0 ? unreadCount + ' sin leer' : 'Todo al día'"></p>
                </div>
            </div>
            <button @click="openDesktop = false" class="p-2 rounded-lg hover:bg-zinc-800 text-zinc-400 hover:text-white transition">
                ...
                <a :href="n.url" 
                   ...
                   class="block p-3 rounded-xl bg-zinc-800/40 hover:bg-zinc-800 border border-zinc-800 hover:border-amber-500/30 transition-all group relative">
                    <div class="flex gap-3">
                        <div class="pt-1.5">
                            <div class="w-2 h-2 rounded-full bg-amber-500 shadow-[0_0_8px_rgba(245,158,11,0.5)] animate-pulse"></div>
                        </div>
        ...
        <div class="p-3 border-t border-zinc-800 bg-zinc-900 shrink-0">
            <a href="{{ route('notificaciones.index') }}" wire:navigate class="flex items-center justify-center w-full py-2 text-xs font-medium text-zinc-400 hover:text-amber-400 hover:bg-zinc-800 rounded-lg transition-colors">
    ...
    <div x-show="openMobile" 
        ...
        <div x-show="openMobile"
            ...
            <div class="p-4 border-b border-zinc-800 flex justify-between items-center shrink-0">
                <h2 class="text-sm font-bold text-white">Notificaciones</h2>
                <button @click="openMobile = false" class="text-zinc-400 p-2 bg-zinc-800 rounded-lg"><svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg></button>
            </div>
            ...
            <div class="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">
                ...
                <template x-for="n in notifications" :key="n.id">
                    <a :href="n.url" @click="markAsRead(n.id)" class="block p-3 rounded-xl bg-zinc-800 border border-zinc-700/50 text-zinc-100">
            ...
            <div class="p-4 border-t border-zinc-800 shrink-0 text-center">
                <a href="{{ route('notificaciones.index') }}" wire:navigate class="text-amber-400 text-sm font-medium">Ver todas las notificaciones</a>
            </div>
        </div>
    </div>
</div>