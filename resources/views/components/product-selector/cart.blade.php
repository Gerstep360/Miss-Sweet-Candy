...
<div class="flex-1 min-h-0 relative" style="height: calc(100vh - 320px);">
    ...
    <div class="absolute top-0 left-0 right-0 h-4 bg-gradient-to-b from-zinc-800 to-transparent pointer-events-none z-10"
         x-show="items.length > 3" x-transition></div>
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                <div class="flex flex-col gap-1">
                                    <div class="flex items-center gap-2 flex-wrap">
                                        <template
                                            ...
                                            <div class="flex items-end gap-2">
                                                <!-- Precio base tachado -->
                                                <span class="text-zinc-400 text-xs line-through">
                                                    $<span x-text="Number(item.precio_base ?? item.precio).toFixed(2)"></span>
                                                    <span class="sr-only">(precio base)</span>
                                                </span>
                                                <!-- Precio vigente -->
                                                <span class="text-amber-300 font-semibold text-sm">
                                                    $<span x-text="Number(item.precio).toFixed(2)"></span> c/u
                                                </span>
                                                <!-- % OFF -->
                                                <span class="text-[10px] font-bold bg-green-500/15 text-green-400 px-1.5 py-0.5 rounded"
                                                      x-text="(() => {
                                    ...
                                    <div class="flex items-center gap-2 flex-wrap">
                                        ...
                                        <template x-if="((item.precio_base ?? item.precio) > item.precio)">
                                            <span class="text-[11px] text-green-400 font-medium bg-green-500/10 px-2 py-0.5 rounded">
                                                Ahorras $
                                                <span
                                                    x-text="(() => {
                                                        const base = Number(item.precio_base ?? item.precio);
...
<div class="p-4 sm:p-6 pt-4 border-t border-zinc-700/50 flex-shrink-0 bg-gradient-to-br from-zinc-800/50 to-zinc-900/50">
    ...
    <!-- Resumen económico -->
    <div class="space-y-3 sm:space-y-4" x-show="items.length > 0" x-transition>
        <!-- Subtotal base y ahorro total -->
        <div class="grid grid-cols-2 gap-3">
            <div class="bg-zinc-800/40 border border-zinc-700/50 rounded-xl p-3">
                <p class="text-zinc-400 text-xs sm:text-sm mb-1">Subtotal (precios base)</p>
            ...
            <div class="bg-green-500/10 border border-green-500/30 rounded-xl p-3">
                <p class="text-green-400 text-xs sm:text-sm mb-1">Ahorro total</p>
        ...
        <div class="bg-gradient-to-br from-amber-500/10 to-amber-600/10 border border-amber-500/30 rounded-xl p-3 sm:p-4">
