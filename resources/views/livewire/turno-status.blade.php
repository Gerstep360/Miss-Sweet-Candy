<div wire:poll.30s="refreshTurno" class="mb-8">
    {{-- Sin turno activo --}}
    @if (! $hayTurno)
            ...
            <div class="flex items-start gap-4">
                ...
                <div class="flex-1">
                    <h3 class="text-xl font-bold text-white mb-2">
                        <span>{{ $turno['cajero']['name'] ?? '—' }}</span> está de turno actualmente
                    </h3>
                    <div class="flex items-center gap-6 text-sm text-zinc-400">
                        <div class="flex items-center gap-2">
                            ...
                            <span>Iniciado {{ $turno['tiempo_transcurrido'] ?? '—' }}</span>
                        </div>
                        <div class="flex items-center gap-2">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            ...
                            <span>Duración: {{ $turno['duracion_formateada'] ?? '—' }}</span>
                        </div>
                    </div>
                    ...
                    <div class="flex-1">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
                            <div class="bg-black/20 rounded-lg p-3">
                                ...
                                <p class="text-white font-semibold">{{ $turno['tiempo_transcurrido'] ?? '—' }}</p>
                            </div>
                            <div class="bg-black/20 rounded-lg p-3">
                                <div class="flex items-center gap-2 text-zinc-400 text-xs mb-1">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <p class="text-white font-semibold">{{ $turno['duracion_formateada'] ?? '—' }}</p>
                            </div>
                            <div class="bg-black/20 rounded-lg p-3">
                                <div class="flex items-center gap-2 text-zinc-400 text-xs mb-1">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                @endphp
                                <p class="text-white font-semibold">Bs. {{ $monto }}</p>
                            </div>
                        </div>
